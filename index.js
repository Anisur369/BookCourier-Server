...
async function run() {
  try {
      ...
      try {
        ...
        if (!book.bookName || !book.author || !book.price) {
          return res.status(400).send({
            error: "Missing required fields: bookName, author, price",
          });
        }
    ...
    app.post("/wishlist", async (req, res) => {
      ...
      if (!bookId || !userId)
        return res.status(400).send({ error: "bookId and userId required" });
    ...
    app.get("/orders/:id", async (req, res) => {
      try {
        ...
        const bookId = ObjectId.isValid(order.bookId)
          ? new ObjectId(order.bookId)
          : order.bookId;
    ...
    app.post("/payment/:orderId", async (req, res) => {
      ...
      try {
        ...
        // Amount in cents
        const amount = order.bookDetails?.price
          ? Math.round(order.bookDetails.price * 100)
          : 0;
    ...
    app.post("/create-checkout-session", async (req, res) => {
      ...
      if (!orderId || !customerEmail)
        return res
          .status(400)
          .send({ error: "Missing orderId or customerEmail" });
    ...
    app.patch("/payment-success", async (req, res) => {
      ...
      try {
        ...
        if (!orderId)
          return res
            .status(400)
            .send({ error: "Order ID missing in session metadata" });
        ...
        if (!order || !book)
          return res.status(404).send({ error: "Order or Book not found" });
        ...
        const paymentRecord = {
          ...
          customer: {
            ...
            photo:
              order.customerDetails?.photo || "https://via.placeholder.com/50",
          },
          product: {
            name: book.bookName,
            price: book.price,
    ...
    app.post("/reviews", async (req, res) => {
      try {
        const { bookId, userId, userName, userPhoto, rating, review, email } =
          req.body;
    ...
    app.get("/admin/stats", async (req, res) => {
      try {
        ...
        const librarians = await usersCollection.countDocuments({
          role: "librarian",
        });
